...
const Nutrition = () => {
  ...
  return (
    <div className="app-shell min-h-screen px-6 py-8 text-foreground">
      <div className="mx-auto max-w-[1540px] space-y-6">
          ...
          <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="app-chip-muted flex items-center gap-2 rounded-2xl px-3 py-2">
                ...
                <div className="min-w-40 text-center"><div className="app-surface-caption text-[11px] uppercase tracking-[0.24em]">Bitacora</div><div className="app-surface-heading text-sm font-semibold">{format(selectedDate, "dd/MM/yyyy")}</div></div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.65fr_0.8fr]">
                ...
                {mealOverview.map(({ meal, entries, subtotal }, index) => (
                  <article key={meal.key} className="app-surface-tile overflow-hidden rounded-[24px]">
                    <div className="flex items-center gap-4 px-5 py-4">
                      ...
                      <button type="button" onClick={() => toggleMeal(meal.key)} className="app-surface-soft app-surface-muted rounded-2xl p-3"><ChevronDown className={cn("h-5 w-5 transition-transform", expandedMeals[meal.key] && "rotate-180")} /></button>
                    </div>
                    {expandedMeals[meal.key] && (
                      <div className="border-t border-border/40 px-5 pb-5 pt-3">
                        <div className="overflow-hidden rounded-2xl border app-surface-soft">
          ...
          <aside className="space-y-5 xl:sticky xl:top-6 xl:self-start">
            ...
            <div className="app-surface-panel rounded-[28px] p-5"><div className="app-surface-caption flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.26em]"><Flame className="h-3.5 w-3.5 text-primary" />Perfil metabolico</div><div className="mt-4 grid gap-3"><div className="grid grid-cols-2 gap-3"><div className="rounded-2xl border app-surface-soft p-3"><div className="app-surface-caption text-[10px] uppercase tracking-[0.24em]">Sexo</div><div className="app-surface-heading mt-2 text-sm">{metabolicProfile?.sex === "female" ? "Femenino" : "Masculino"}</div></div><div className="rounded-2xl border app-surface-soft p-3"><div className="app-surface-caption text-[10px] uppercase tracking-[0.24em]">Edad</div><div className="app-surface-heading mt-2 text-sm">{metabolicProfile?.age ?? "--"} anos</div></div></div><div className="grid grid-cols-2 gap-3"><div className="rounded-2xl border app-surface-soft p-3"><div className="app-surface-caption text-[10px] uppercase tracking-[0.24em]">Peso</div><div className="app-surface-heading mt-2 text-sm">{formatMetric(metabolicProfile?.weightKg, " kg", 1)}</div></div><div className="rounded-2xl border app-surface-soft p-3"><div className="app-surface-caption text-[10px] uppercase tracking-[0.24em]">Altura</div><div className="app-surface-heading mt-2 text-sm">{formatMetric(metabolicProfile?.heightCm, " cm", 0)}</div></div></div><div className="rounded-2xl border app-surface-soft p-3"><div className="app-surface-caption text-[10px] uppercase tracking-[0.24em]">Actividad</div><div className="app-surface-heading mt-2 text-sm">{ACTIVITY_LABELS[metabolicProfile?.activityLevel ?? "moderate"] ?? "--"}</div></div><div className="rounded-2xl border app-surface-soft p-3"><div className="app-surface-caption text-[10px] uppercase tracking-[0.24em]">Objetivo</div><div className="app-surface-heading mt-2 text-sm">{GOAL_LABELS[metabolicProfile?.goalType ?? "maintain"] ?? "--"}</div></div><Button asChild className="rounded-2xl bg-primary text-primary-foreground hover:bg-primary/90"><Link to="/fitness-profile">Abrir Perfil Fitness</Link></Button></div></div>
          </aside>
        </div>
      </div>
      <Dialog open={profilesDialogOpen} onOpenChange={setProfilesDialogOpen}><DialogContent className="app-dialog-surface max-w-xl"><DialogHeader><DialogTitle>{editingProfile ? "Editar perfil nutricional" : "Crear perfil nutricional"}</DialogTitle></DialogHeader><div className="space-y-4"><div className="space-y-2"><Label>Nombre</Label><Input value={profileName} onChange={(event) => setProfileName(event.target.value)} placeholder="Ej. Pierna, Torso, Descanso" className="app-input-surface" /></div><div className="space-y-2"><Label>Arquetipo</Label><Select value={profileArchetype} onValueChange={(value) => setProfileArchetype(value as NutritionDayArchetype)}><SelectTrigger className="app-input-surface"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(NUTRITION_ARCHETYPE_META).map(([key, meta]) => <SelectItem key={key} value={key}>{meta.label}</SelectItem>)}</SelectContent></Select><div className="app-surface-soft rounded-2xl p-4 text-sm">{NUTRITION_ARCHETYPE_META[profileArchetype].description}</div></div><label className="app-surface-muted flex items-center gap-3 text-sm"><Checkbox checked={profileIsDefault} onCheckedChange={(checked) => setProfileIsDefault(Boolean(checked))} />Marcar como perfil predeterminado</label></div><DialogFooter><Button variant="outline" onClick={() => setProfilesDialogOpen(false)} className="app-outline-button">Cancelar</Button><Button onClick={() => saveProfileMutation.mutate()} disabled={saveProfileMutation.isPending} className="bg-primary text-primary-foreground hover:bg-primary/90">Guardar perfil</Button></DialogFooter></DialogContent></Dialog>
      <Dialog open={mealDialogOpen} onOpenChange={setMealDialogOpen}><DialogContent className="app-dialog-surface max-w-3xl"><DialogHeader><DialogTitle className="text-xl font-bold">Agregar comida - {MEAL_SECTIONS.find((m) => m.key === activeMeal)?.label}</DialogTitle></DialogHeader><div className="space-y-4"><div className="grid grid-cols-[200px_1fr] gap-4"><div className="space-y-2"><Label className="app-surface-muted">Modo de carga</Label><Select value={mode} onValueChange={(value) => setMode(value as AddMode)}><SelectTrigger className="app-input-surface"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="manual">Carga manual</SelectItem><SelectItem value="database">Base de alimentos</SelectItem><SelectItem value="favorite">Favoritos</SelectItem><SelectItem value="yesterday">Duplicar de ayer</SelectItem><SelectItem value="recent">Recientes</SelectItem></SelectContent></Select></div><div className="app-panel-block app-surface-muted rounded-2xl px-4 py-3 text-sm"><div className="app-surface-heading font-medium">Contexto activo</div><div className="mt-1">Comida destino: {MEAL_SECTIONS.find((m) => m.key === activeMeal)?.label}</div><div className="mt-1">Perfil del dia: {effectiveProfileLabel}</div></div></div>{mode === "database" && <div className="app-panel-block space-y-3 rounded-2xl p-4"><div className="grid grid-cols-[1fr_220px] gap-3"><Input value={searchFood} onChange={(e) => setSearchFood(e.target.value)} placeholder="Buscar alimento..." className="app-input-surface" /><Select value={foodCategory} onValueChange={setFoodCategory}><SelectTrigger className="app-input-surface"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todas</SelectItem>{(categoriesQuery.data || []).map((category) => <SelectItem key={category} value={category}>{category}</SelectItem>)}</SelectContent></Select></div><Select value={selectedFoodDatabaseId} onValueChange={setSelectedFoodDatabaseId}><SelectTrigger className="app-input-surface"><SelectValue placeholder="Selecciona alimento" /></SelectTrigger><SelectContent>{(foodSearchQuery.data || []).map((food) => <SelectItem key={food.id} value={food.id}>{food.food_name} ({food.calories} kcal/{food.serving_size}{food.serving_unit})</SelectItem>)}</SelectContent></Select><div className="grid grid-cols-[1fr_120px] gap-3"><Input value={consumedAmount} onChange={(e) => setConsumedAmount(e.target.value)} type="number" min="0" placeholder="Cantidad consumida" className="app-input-surface" /><div className="app-panel-block app-surface-muted flex items-center justify-center rounded-xl text-sm">{(foodSearchQuery.data || []).find((row) => row.id === selectedFoodDatabaseId)?.serving_unit || "g"}</div></div>{selectedFoodPreview && <div className="rounded-2xl border border-cyan-400/15 bg-cyan-400/5 p-4 text-sm text-foreground/80">Vista previa: {selectedFoodPreview.calories} kcal | P {selectedFoodPreview.protein_g} | C {selectedFoodPreview.carbs_g} | G {selectedFoodPreview.fat_g}</div>}</div>}{mode === "manual" && <div className="app-panel-block space-y-3 rounded-2xl p-4"><Input value={foodName} onChange={(e) => setFoodName(e.target.value)} placeholder="Nombre alimento" className="app-input-surface" /><div className="grid grid-cols-[1fr_110px] gap-3"><Input value={servingSize} onChange={(e) => setServingSize(e.target.value)} type="number" min="0" placeholder="Porcion" className="app-input-surface" /><Input value={servingUnit} onChange={(e) => setServingUnit(e.target.value)} placeholder="unidad" className="app-input-surface" /></div><div className="grid grid-cols-4 gap-3"><Input value={calories} onChange={(e) => setCalories(e.target.value)} type="number" min="0" placeholder="kcal" className="app-input-surface" /><Input value={protein} onChange={(e) => setProtein(e.target.value)} type="number" min="0" placeholder="Proteina g" className="app-input-surface" /><Input value={carbs} onChange={(e) => setCarbs(e.target.value)} type="number" min="0" placeholder="Carbs g" className="app-input-surface" /><Input value={fat} onChange={(e) => setFat(e.target.value)} type="number" min="0" placeholder="Grasas g" className="app-input-surface" /><Input value={fiber} onChange={(e) => setFiber(e.target.value)} type="number" min="0" placeholder="Fibra g" className="app-input-surface" /><Input value={sugar} onChange={(e) => setSugar(e.target.value)} type="number" min="0" placeholder="Azucar g" className="app-input-surface" /><Input value={sodium} onChange={(e) => setSodium(e.target.value)} type="number" min="0" placeholder="Sodio mg" className="app-input-surface" /><Input value={potassium} onChange={(e) => setPotassium(e.target.value)} type="number" min="0" placeholder="Potasio mg" className="app-input-surface" /></div><label className="app-surface-muted flex items-center gap-2 text-sm"><input type="checkbox" checked={saveAsFavorite} onChange={(e) => setSaveAsFavorite(e.target.checked)} />Guardar en favoritos</label></div>}{mode === "favorite" && <Select value={selectedFavoriteId} onValueChange={setSelectedFavoriteId}><SelectTrigger className="app-input-surface"><SelectValue placeholder="Selecciona favorito" /></SelectTrigger><SelectContent>{(favoritesQuery.data || []).map((item) => <SelectItem key={item.id} value={item.id}>{item.name} ({item.calories} kcal)</SelectItem>)}</SelectContent></Select>}{mode === "yesterday" && <Select value={selectedYesterdayId} onValueChange={setSelectedYesterdayId}><SelectTrigger className="app-input-surface"><SelectValue placeholder="Selecciona comida de ayer" /></SelectTrigger><SelectContent>{yesterdayEntries.map((item) => <SelectItem key={item.id} value={item.id}>{item.food_name} ({item.calories} kcal)</SelectItem>)}</SelectContent></Select>}{mode === "recent" && <Select value={selectedRecentId} onValueChange={setSelectedRecentId}><SelectTrigger className="app-input-surface"><SelectValue placeholder="Selecciona reciente" /></SelectTrigger><SelectContent>{(recentQuery.data || []).map((item) => <SelectItem key={item.id} value={item.id}>{item.food_name} ({item.calories} kcal)</SelectItem>)}</SelectContent></Select>}</div><DialogFooter><Button variant="outline" onClick={() => setMealDialogOpen(false)} className="app-outline-button">Cancelar</Button><Button onClick={handleAddEntry} disabled={addMutation.isPending} className="bg-primary text-primary-foreground hover:bg-primary/90">Guardar</Button></DialogFooter></DialogContent></Dialog>
    </div>
  );
};
