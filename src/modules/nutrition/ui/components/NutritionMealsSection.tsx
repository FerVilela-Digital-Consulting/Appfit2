  ...
  return (
    <div className="app-surface-panel rounded-[24px] sm:rounded-[28px]">
      <div className="flex flex-col gap-3 border-b border-border/40 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-5">
        ...
        <Button onClick={() => onOpenMealDialog("breakfast")} className="w-full rounded-2xl bg-primary px-4 text-primary-foreground hover:bg-primary/90 sm:w-auto"><CirclePlus className="mr-2 h-4 w-4" />Anadir nueva comida</Button>
      </div>
        ...
        {mealOverview.map(({ meal, entries, subtotal }, index) => (
          <article key={meal.key} className="app-surface-tile overflow-hidden rounded-[20px] sm:rounded-[24px]">
            <div className="grid gap-4 px-4 py-4 sm:px-5 xl:grid-cols-[minmax(0,220px)_minmax(0,1fr)_auto] xl:items-center">
              ...
              <button type="button" onClick={() => onToggleMeal(meal.key)} className="app-surface-soft app-surface-muted justify-self-end rounded-2xl p-3"><ChevronDown className={cn("h-5 w-5 transition-transform", expandedMeals[meal.key] && "rotate-180")} /></button>
            </div>
            {expandedMeals[meal.key] && (
              <div className="border-t border-border/40 px-4 pb-4 pt-3 sm:px-5 sm:pb-5">
                      ...
                      {entries.length === 0 ? (
                        ...
                        entries.map((entry) => (
                          <tr key={entry.id} className="border-b border-border/40 last:border-b-0">
                            ...
                            <td className="px-4 py-3 text-right"><Button variant="ghost" size="icon" onClick={() => onDeleteEntry(entry.id)} className="app-surface-muted rounded-xl hover:bg-red-500/10 hover:text-red-300"><Trash2 className="h-4 w-4" /></Button></td>
                          </tr>
                        ))
                      )}
                    </tbody>
                    ...
                    entries.map((entry) => (
                      <div key={entry.id} className="rounded-2xl border border-border/50 px-4 py-3">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <Button variant="ghost" size="icon" onClick={() => onDeleteEntry(entry.id)} className="app-surface-muted rounded-xl hover:bg-red-500/10 hover:text-red-300"><Trash2 className="h-4 w-4" /></Button>
                        </div>
                ...
                <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
                  <button type="button" onClick={() => onOpenMealDialog(meal.key, "manual")} className="app-surface-soft rounded-2xl px-4 py-3 text-sm font-medium"><PencilLine className="mr-2 inline h-4 w-4" />Carga manual</button>
                  <button type="button" onClick={() => onOpenMealDialog(meal.key, "database")} className="app-surface-soft rounded-2xl px-4 py-3 text-sm font-medium"><Database className="mr-2 inline h-4 w-4" />Buscar alimento</button>
                  <button type="button" onClick={() => onOpenMealDialog(meal.key, "favorite")} className="app-surface-soft rounded-2xl px-4 py-3 text-sm font-medium"><ShieldPlus className="mr-2 inline h-4 w-4" />Usar favorito</button>
                </div>
              </div>
            )}
          </article>
